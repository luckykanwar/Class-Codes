...
//     while(j<numberOfNodes){
    while(j<100){
         ...
         for(int z =0; z<numberNodes; z++)
         {
            if(z!=globalId)
            {
                sendingPacket.arraySend[z] = array[z];
            }
         }

         array[globalId] = sendingPacket.clock;
         sendingPacket.arraySend[globalId] = clock;
...
//         for(int k =0;k<numberNodes;k++)
          ...
          {
            if(listeningPacket.arraySend[i]>=array[i] && i!=globalId)
            {
                array[i] =listeningPacket.arraySend[i];
            }
          }
//         {
