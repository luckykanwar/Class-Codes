...
main(int argc, char *argv[])
{
     ...
     int fd, nbytes,addrlen, newsocket;
     struct ip_mreq mreq;
...
/*** END OF MODIFICATION TO ORIGINAL */
     ...
     while (1) {
	  addrlen=sizeof(addr);
	    ...
	    if(1){
        printf("Calling the send time function on the client\n");
        ...
        if( (sent_bytes = sendto(fd,"send_time",sizeof("send_time"),0,(struct sockaddr *) &addr,
		     sizeof(addr)) < 0 )) {
		     printf("Something wrong while sending the send time request\n");
            perror("send error");
            return -1;
        }
        counter = counter + increaseCounter;
        printf("Inside the calling location for time interval! Local counter is : %d\n", counter);
	  }

      newsocket = socket(AF_INET,SOCK_DGRAM,0);
      setsockopt(newsocket,SOL_SOCKET,SO_REUSEADDR,&yes,sizeof(yes));
      bind(newsocket,(struct sockaddr *) &addr,sizeof(addr));
	  if ((nbytes=recvfrom(newsocket,msgbuf,MSGBUFSIZE,0,
        ...
        printf("Received message from the sender location\n");
        puts(msgbuf);
        counter = counter + increaseCounter;
        printf("Local counter in listener is : %d\n", counter);
        printf("Local counter in listener is : %d\n", counter%10);
	  }
	  counter = counter + increaseCounter;
	  printf("Local counter in listener outside of if conditions is : %d\n", counter);
     }
}
